...
public class ParrySkill extends Skill {
    ...
    public void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        if (!(PlayerSkillManager.getSkillInstance(serverPlayer, SkillRegistry.PARRY.getSkillId()) instanceof ParrySkill parrySkill)) return;
    ...
    public void onLivingAttack(LivingAttackEvent event) {
        ...
        if (!(PlayerSkillManager.getSkillInstance(serverPlayer, SkillRegistry.PARRY.getSkillId()) instanceof ParrySkill parrySkill)) return;
        if (!parrySkill.isParryReady) return;
        if (!(event.getEntity().getUseItem().getItem() instanceof ShieldItem)) return;
